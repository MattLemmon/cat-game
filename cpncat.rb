...
#$LOAD_PATH.unshift File.join(File.expand_path(__FILE__), "..", "cat-game", "media", "cpnmedia")
require 'chingu'
include Gosu
...
#    @animation.frame_names = { :scan => 0..0, :up => 1..2, :down => 3..4, :left => 1..2, :right => 3..4 }
    ...
    @standing, @walk1, @walk2, @jump, @wall_climb, @blink=
      *Image.load_tiles(window, "media/testcat2.png", 50, 50, false)
...
#      puts (milliseconds / 175 % 4)
  ...
  def try_to_jump
    ...
    if @wall != 0
#      if @map.solid?( @x + 11, @y ) or @map.solid?( @x - 11, @y ) then
