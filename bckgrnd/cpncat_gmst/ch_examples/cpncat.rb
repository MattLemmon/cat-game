...
class CptnRuby
  ...
  def initialize(window, x, y)
    ...
    @standing, @walk1, @walk2, @jump, @wall_climb=
      *Image.load_tiles(window, "media/droid.bmp", 280, 250, false)
  ...
  def draw(screen_x, screen_y)
    ...
    @cur_image.draw(@x - screen_x + offs_x, @y - screen_y - 99, 0, factor, 1.0)
...
class Map
  ...
  def initialize(window, filename)
    ...
    @tileset = Image.load_tiles(window, "media/CRTileset.png", 60, 60, true)
...
# CpnCat GAMESTATE CLASS
=begin
class CpnCat < Chingu::GameState
  def initialize
    super  #(640,480,false)           # leave it blank and it will be 800,600,non fullscreen
    self.input = { :escape => :exit } # exits example on Escape
    
    @player = Player.create(:x => 200, :y => 200, :image => Image["cat1.png"])
